...
var buildCable = function(elemCount,elemRadius,rad,cablen,offset=1,inner=0)
{
   ...
   for(var k =0;k<elemCount;k++)
   {
       ...
       if(inner==0){
     //  createTubes(8,innerElemsRadius/(elemCount*0.8) ,10,'','',elemCoords.x/2.5,elemCoords.y,elemCoords.z);
     //  createTubes(4,innerElemsRadius/(elemCount*0.5) ,10,'','',elemCoords.x/2.5,elemCoords.y,elemCoords.z,0.1);
...
console.log(bodyDiameter);

if(getUriParam('secondBody',location.href))
{
 secondBodyMaterial = blackLightPBRTextured;
...
{
 scene.meshes[x].freezeWorldMatrix();
 //scene.meshes[x].convertToUnIndexedMesh();
}
