...
var createTubesUnOpt = function(count,radius = 0.2,steps=5,mod=0.5,mat = copperPBRTextured,positionX,positionY,positionZ,innerRad = 0.21,length=3,invert=0,skipStep=1,curveStep=2,xMod=0,yMod=0.38,zMod=0,path)
{
    if(typeof path == 'undefined' || path=='') {
    ...
    for(var i=1; i<=count;i++) {
        ...
        var realTubeCoordsX = positionX;
        var realTubeCoordsY = positionY;
        var realTubeCoordsZ = positionZ;
     //   path = otherCurve('', innerRad, length, steps + 0.5, mod);
    //    createTubes(count, 0.05, 4.3, 0.01, copperPBRTextured, positionX, realTubeCoordsY, realTubeCoordsZ, 0.3, 6, path);
    }
}
...
minMod = parseFloat(minMod);
 ...
 createCableFilling(0,bodyLength,secondBodyMaterial,bodyDiameter-0.6,bodyDiameter-0.6,bodyDiameter-0.6,
     '','',0,bodyLength*1.2,0,0,'','',cutFillings,'Дополнительная оболочка');
 createHelix(blackLightPBRTexturedHelix,bodyDiameter/3.8  ,20,20,0,0,50,'Лента из прорезиненной ткани');
 createHelix(plenkaPBRTextured,bodyDiameter/4.3,20,30,0,0,50);*/
}else {
// createHelix(plenkaPBRTextured,bodyDiameter/3.9,20,30,0,0,50);
}
if(innerElemsRadius>1.5){
 //bodyDiameter = bodyDiameter + 0.5;
}
...
//createCableFilling(1,bodyLength*1.5,blackLightPBRTextured,4,4,4,'','','',
  //  bodyLength*1.6,0,0,'','','','Изоляция упрочняющего сердечника');
//createCableFilling(1,bodyLength*1.7,ropePBR,3,3,3,'','','',
...
// Positions the camera overwriting alpha, beta, radius
  ...
  if (doonce == 62) {
   ...
   xhr.send("name=total"+eval(totalElems)+"firstrow"+elemsFirstRow+";secrow"+elemsSecondRow+";cut"+innerElemsRadius+"&picfile=" + previewPic);
  }
  if (doonce == 63) {
   camera.setPosition(new BABYLON.Vector3(60, 0, 60));
  }
...
//newBuild(elemsFirstRow,bodyDiameter/modFirstRow,cableFillingLength,2,0.8,0,1.5);
//newBuild(elemsSecondRow,bodyDiameter/modSecondRow,cableFillingLength*2.33,2,0.92,1,3);
/*
createTubesUnOpt(7,0.6,3.3,0.1,plasticPBRGrey,1,1,6,1.5);
createTubesUnOpt(7,0.3,3.36,0.1,copperPBRTextured,1,1,6,1.5);

createHelix(plenkaPBRTextured,1.3,20,30,1,6,50);
createCableFilling(0,11,blackPBRTextured,5.5,5.5,5.5,'','','',
    10,1,6,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,11,blackLightPBRTextured,1.5,1.5,1.5,'','','',
    16,1,6,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,13,ropePBR,1.3,1.3,1.3,'','','',
    16,1,6,'','',cableFillings,'Упрочняющий сердечник');

createTubesUnOpt(9,0.6,5.6,0.1,plasticPBRGrey,-13,1,15,1.8,3,0,4.5);
createTubesUnOpt(9,0.3,5.7,0.1,copperPBRTextured,-13,1,15,1.8,3,0,4.5);


createHelix(plenkaPBRTextured,1.3,20,30,1,14.5,50);
createCableFilling(0,11,blackPBRTextured,5.5,5.5,5.5,'','','',
    10,1,15,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,11,blackLightPBRTextured,1.5,1.5,1.5,'','','',
    16,1,15,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,13,ropePBR,1.3,1.3,1.3,'','','',
    16,1,15,'','',cableFillings,'Упрочняющий сердечник');

createTubesUnOpt(5,0.6,4.3,0.1,plasticPBRGrey,1,1,22,3,3,0);
createTubesUnOpt(7,0.6,4.9,0.1,plasticPBRGrey,1,1,22,1.8);
createTubesUnOpt(5,0.3,4.35,0.1,copperPBRTextured,1,1,22,3,3,0);
createTubesUnOpt(7,0.3,5,0.1,copperPBRTextured,1,1,22,1.8);

createHelix(plenkaPBRTextured,1.9,20,40,1,21.5,50);
createCableFilling(0,22,blackPBRTextured,8.5,8.5,8.5,'','','',
    10,1,22,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,22,blackLightPBRTextured,1.5,1.5,1.5,'','','',
    21,1,22,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,24,ropePBR,1.3,1.3,1.3,'','','',
    21,1,22,'','',cableFillings,'Упрочняющий сердечник');

/*
createTubesUnOpt(9,0.6,4.9,0.1,plasticPBRGrey,1,1,38,4.5,3,0,3);
createTubesUnOpt(6,0.6,5.3,0.1,plasticPBRGrey,1,1,38,3.3,3,0,3);
createTubesUnOpt(4,0.6,5.8,0.1,plasticPBRGrey,1,1,38,1.8,3,1,3);
createTubesUnOpt(9,0.3,5,0.1,copperPBRTextured,1,1,38,4.5,3,0,3);
createTubesUnOpt(6,0.3,5.4,0.1,copperPBRTextured,1,1,38,3.3,3,0,3);
createTubesUnOpt(4,0.3,5.9,0.1,copperPBRTextured,1,1,38,1.8,3,1,3);
*/
/*
createTubesUnOpt(7,0.6,4.3,0.1,plasticPBRGrey,1,1,38,4.1,3,0,3);
createTubesUnOpt(6,0.6,5.8,0.1,plasticPBRGrey,1,1,38,3,3,0,3);
createTubesUnOpt(6,0.6,7.2,0.1,plasticPBRGrey,1,1,38,1.8,3,0,3);
createTubesUnOpt(7,0.3,4.4,0.1,copperPBRTextured,1,1,38,4.1,3,0,3);
createTubesUnOpt(6,0.3,5.9,0.1,copperPBRTextured,1,1,38,3,3,0,3);
createTubesUnOpt(6,0.3,7.3,0.1,copperPBRTextured,1,1,38,1.8,3,0,3);


createHelix(plenkaPBRTextured,3,20,40,1,37.5,50);
createCableFilling(0,22,blackPBRTextured,12.5,12.5,12.5,'','','',
    10,1,38,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,36,blackLightPBRTextured,1.5,1.5,1.5,'','','',
    23,1,38,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,38,ropePBR,1.3,1.3,1.3,'','','',
    23,1,38,'','',cableFillings,'Упрочняющий сердечник');


createTubesUnOpt(7,1,3.3,0.1,plasticPBRGrey,1,1,-7,2.5);
createTubesUnOpt(7,0.7 ,3.36,0.1,copperPBRTextured,1,1,-7,2.5);
createHelix(plenkaPBRTextured,1.3,20,30,1,-7,50);
createCableFilling(0,11,blackPBRTextured,8.5,8.5,8.5,'','','',
    10,1,-7,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,11,blackLightPBRTextured,2.5,2.5,2.5,'','','',
    16,1,-7,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,13,ropePBR,2.3,2.3,2.3,'','','',
    16,1,-7,'','',cableFillings,'Упрочняющий сердечник');


createTubesUnOpt(9,1,4.1,0.1,plasticPBRGrey,-3,1,-18,3.0,3,0,0,2,0,0,0.8);
createTubesUnOpt(9,0.7,4.2,0.1,copperPBRTextured,-3,1,-18,3.0,3,0,0,2,0,0,0.8);

createHelix(plenkaPBRTextured,2.7,20,30,1,-18,50);
createCableFilling(0,11,blackPBRTextured,11,11,11,'','','',
    10,1,-18,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,11,blackLightPBRTextured,2.5,2.5,2.5,'','','',
    16,1,-18,'','',cableFillings,'Упрочняющий сердечник');
createCableFilling(0,13,ropePBR,2.3,2.3,2.3,'','','',
    16,1,-18,'','',cableFillings,'Упрочняющий сердечник');
...
var buildRows = function(elemsToCut)
{
    ...
    while(elemsLeft>0) {
        it++;
        if(it>=7) { buildRow[rows] = it;  it = 0; rows = rows + 1;}
...
if(getTotalElems<9 && innerElemsRadius!=2.5) {
    bodyResultDiameter = Math.ceil(2.6 * getRowsToBuild.length);
...
{
    createTubesUnOpt(getRowsToBuild[i],options.radius,6.25-(i*0.95),options.mod,options.material,0,0,0,1.8+(i*options.innerRad),3,0,0,2,0,0,0);
    createTubesUnOpt(getRowsToBuild[i],options.innerElemsRadius,6.35-(i*0.95),options.mod,copperPBRTextured,0,0,0,1.8+(i*options.innerRad),3,0,0,2,0,0,0);
}

var helixDiameter = 2.7;
if(getTotalElems!=9){
    helixDiameter = 2.7 - ((19 - getTotalElems)/10);
}else {
    helixDiameter = Math.ceil(2.7 - ((19 - getTotalElems)/10));
}

if(!getUriParam('secondBody',location.href)){
    createCableFilling(0, 33, blackPBRTextured, bodyResultDiameter, bodyResultDiameter, bodyResultDiameter, '', '', '',
        0, 0, 0, '', '', cableFillings, 'Оболочка кабеля');
    createHelix(plenkaPBRTextured, helixDiameter, 20, 40, 0, 0, 50);
}else {
    createCableFilling(0, 33, blackPBRTextured, bodyResultDiameter+0.55, bodyResultDiameter+0.55, bodyResultDiameter+0.55, '', '', '',
        -1, 0, 0, '', '', cableFillings, 'Оболочка кабеля');

    createHelix(blackLightPBRTexturedHelix, helixDiameter-0.1, 20, 40, 0, 0, 50);
    if(getTotalElems==7)
    {
        helixDiameter -= 0.34;
    }
    createHelix(plenkaPBRTextured, helixDiameter+0.34, 20, 30, 0, 0, 40);
    createCableFilling(0, 33, blackLightPBRTextured, bodyResultDiameter-0.5, bodyResultDiameter-0.5, bodyResultDiameter-0.5, '', '', '',
        3, 0, 0, '', '', cableFillings, 'Оболочка кабеля');
}
...
if(elemsFirstRow==0)
{
    bodyDiameter  = bodyDiameter/2;
}

bodyDiameter+=1.2;

//createMainBody(bodyLength,bodyDiameter,bodyDiameter,bodyDiameter,bodyMaterial);
//createHelix(plenkaPBRTextured,bodyDiameter/4.1,20,25,0,0,50);
