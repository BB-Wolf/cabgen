...
var createTubesUnOpt = function(count,radius = 0.2,steps=5,mod=0.5,mat = copperPBRTextured,positionX,positionY,positionZ,innerRad = 0.21,length=3,invert=0,skipStep=1,curveStep=2,xMod=0,yMod=0.38,zMod=0,path,bdm,storage=[])
{
...
for(var i=0; i<getRowsToBuild.length;i++) {
    ...
    if (getTotalElems == 9 && innerElemsRadius == 2.5) {
        options.zMod = 0.7;
        options.skipStep = 2, options.length = 2.5
    }
    ...
    if (getTotalElems != 12) {
        ...
        createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius, 6.35, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod, options.zMod);
    }
}

console.log(tubeFillings);
