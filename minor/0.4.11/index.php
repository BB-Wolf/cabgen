...
var buildCable = function(elemCount,elemRadius,rad,cablen,offset=1,inner=0)
{
   ...
   for(var k =0;k<elemCount;k++)
   {
       ...
       if(inner==0){
       createTubes(8,innerElemsRadius/(elemCount*0.5) ,10,'','',elemCoords.x/2.5,elemCoords.y,elemCoords.z);
       createTubes(4,innerElemsRadius/(elemCount*0.8) ,10,'','',elemCoords.x/2.5,elemCoords.y,elemCoords.z,0.1);
       }else {
...
var createTubes = function(count,radius = 0.2,steps=10,mod=0.5,mat = copperPBRTextured,positionX,positionY,positionZ,innerRad = 0.21)
{
    for(var i=1; i<=count;i++)
    {
        var tube = BABYLON.MeshBuilder.CreateTube("Токопроводящая жила", {
            ...
            cap: BABYLON.Mesh.CAP_ALL,tesselation:8
        }, scene);
...
minMod = parseFloat(minMod);console.log('minmod:'+minMod);
 createCableFilling(0,bodyLength,secondBodyMaterial,bodyDiameter-0.2,bodyDiameter-0.5,bodyDiameter-0.5,
...
{
 scene.meshes[i].material = plasticPBRGrey;
 scene.meshes[i].material.wireframe = true;
}
scene.meshes[1].material = blackPBRTextured;
scene.meshes[1].material.wireframe = true;*/
