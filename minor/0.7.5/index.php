...
var createTubesUnOpt = function(count,radius = 0.2,steps=5,mod=0.5,mat = copperPBRTextured,positionX,positionY,positionZ,innerRad = 0.21,length=3,invert=0,skipStep=1,curveStep=2,xMod=0,yMod=0.38,zMod=0,path,bdm,storage=[],curveMod=3)
{
...
for(var i=0; i<getRowsToBuild.length;i++) {
    ...
    if (getTotalElems != 12 && getTotalElems!=9) {
        createTubesUnOpt(getRowsToBuild[i], options.radius, 6.25 - (i * 0.95), options.mod, options.material, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod, options.zMod, '','',tubeFillings);
       // createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius, 6.35 - (i * 0.95), options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod, options.zMod);
       if(innerElemsRadius==1.5) {
           var gtc = getAngle(10);
           for (var gcc = 0; gcc <= 10; gcc++) {
               var gco = getCoords(0.1, gtc * gcc);
               createTubesUnOpt(getRowsToBuild[i], 0.05, 6.45 - (i * 0.95), options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, 4, 2, options.xMod, options.yMod + gco.y, options.zMod + gco.z, '', '', false, 3);
           ...
           for (var gcc = 0; gcc <= 5; gcc++) {
               var gco = getCoords(0.1, gtc * gcc);
               createTubesUnOpt(getRowsToBuild[i], 0.09, 6.45 - (i * 0.95), options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, 4, 2, options.xMod, options.yMod + gco.y, options.zMod + gco.z, '', '', false, 3);
           }
           var gtc = getAngle(10);
           for (var gcc = 0; gcc <= 10; gcc++) {
               var gco = getCoords(0.2, gtc * gcc);
           ...
           for (var gcc = 0; gcc <= 20; gcc++) {
               var gco = getCoords(0.3, gtc * gcc);
               createTubesUnOpt(getRowsToBuild[i], 0.09, 6.45 - (i * 0.95), options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, 4, 2, options.xMod, options.yMod + gco.y, options.zMod + gco.z, '', '', false, 3);
           }
       }
    } else {
        createTubesUnOpt(getRowsToBuild[i], options.radius, 6.25, options.mod, options.material, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod, options.zMod, '','',tubeFillings);

      var gtc = getAngle(10);
      for(var gcc=0;gcc<=10;gcc++) {
          var gco = getCoords(0.1,gtc*gcc);
          createTubesUnOpt(getRowsToBuild[i], 0.05, 6.55, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod + gco.y, options.zMod + gco.z, '', '', false,3);
          var gco = getCoords(0.2,gtc*gcc);
          createTubesUnOpt(getRowsToBuild[i], 0.05, 6.55, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod + gco.y, options.zMod + gco.z, '', '', false,3);
      }

     /*   createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius/4, 6.55, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod, options.zMod,'','');
        createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius/4, 6.55, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod+0.1, options.zMod,'','');
        createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius/4, 6.55, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod-0.1, options.zMod,'','');

        createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius/4, 6.55, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod-0.1, options.zMod-0.1,'','');
        createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius/4, 6.55, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod+0.1, options.zMod-0.1,'','');
        createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius/4, 6.55, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod-0.1, options.zMod+0.1,'','');
        createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius/4, 6.55, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod+0.1, options.zMod+0.1,'','');

        createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius/4, 6.55, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod+0.2, options.zMod-0.2,'','');
        createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius/4, 6.55, options.mod, copperPBRTextured, 0, 0 + options.fixY, 0 + options.fixZ, options.innerRadBase + (i * options.innerRad), options.length, options.invert, options.skipStep, 2, options.xMod, options.yMod-0.2, options.zMod+0.2,'','');
   */
    }


 //    createTubesUnOpt(1, 0.1, 6.35, options.mod, copperPBRTextured, lastCoords[0][50].x, lastCoords[0][50].y*2, lastCoords[0][50].z , 1.8, options.length, options.invert, options.skipStep, 2, 0, 0, 0,linearCurve(5),'');
...
if(!getUriParam('secondBody',location.href)){
    ...
    if(getTotalElems==12 && innerElemsRadius == 2.5) { scaleParamFix = 1.7; bodyResultDiameter+=1;}
