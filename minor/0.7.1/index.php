...
for(var i=0; i<getRowsToBuild.length;i++)
{
    ...
    if(getTotalElems==12){ helixDiameter = 0.8; scaleParamFix = 0.9; bodyResultDiameter +=1.; options.innerRad = 1.8; options.innerRadBase = 1.8; options.skipStep=1; options.zMod =-0.8; options.yMod = 0.69}
    ...
    if(getTotalElems==7 && innerElemsRadius == 2.5) { options.zMod = 0; bodyResultDiameter+=1.5; options.skipStep=2;}
        createTubesUnOpt(getRowsToBuild[i], options.radius, 6.25 - (i * 0.95), options.mod, options.material, 0, 0, 0, options.innerRadBase + (i * options.innerRad), options.length, 0, options.skipStep, 2, options.xMod, options.yMod, options.zMod, '');
        createTubesUnOpt(getRowsToBuild[i], options.innerElemsRadius, 6.35 - (i * 0.95), options.mod, copperPBRTextured, 0, 0, 0, options.innerRadBase + (i * options.innerRad), options.length, 0, options.skipStep, 2, options.xMod, options.yMod, options.zMod);
}
